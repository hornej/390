...
#define TEST_FILTER_FLOATING_POINT_EPSILON 1.0E-16L
...
double filterTest_computeGoldenPowerValue(queue_t* q) {
  ...
  for (uint16_t i=0; i<queue_size(q); i++) {  // Iterate over all elements.
...
bool filterTest_runPowerTest() {
  ...
  for (uint16_t i=0; i<FILTER_FREQUENCY_COUNT; i++) {
    ...
    queue_size_t size;
    if ((size = queue_size(q)) != OUTPUT_QUEUE_SIZE) {
      printf("Output queue for queue number %d is incorrect (%ld). It should be %d\n\r", i, size, OUTPUT_QUEUE_SIZE);
    ...
    double testValue = filter_computePower(i, true, false);  // false, false = no force, no debug print.
  ...
  for (uint32_t loopCount=0; loopCount<TEST_INCREMENTAL_LOOP_COUNT; loopCount++) {
    for (uint16_t i=0; i<FILTER_FREQUENCY_COUNT; i++) {
      ...
      if (fabs(testValue - goldenValue) > TEST_PASS_EPSILON) {  // See if the value is in error beyond some epsilon.
        ...
        printf("filter_runPowerTest failed for index: %d\n\rgolden value:          %lf\n\rfilter_computePower(): %lf\n\r",
            i, goldenValue, testValue);
        printf("Difference between golden value and incrementally computed value: %le\n\r",
